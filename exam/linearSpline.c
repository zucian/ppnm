...
double linear_spline(int numberOfPoints, double *points, double *functionValues, double evaluationPoints)
{
    ...
    assert((numberOfPoints > 1) && (evaluationPoints >= points[0]) && (evaluationPoints <= points[numberOfPoints - 1]));

    //Binary search to find subinterval for evaluation point
    int whichInterval = binarySearch(numberOfPoints, points, evaluationPoints);
...
{
    //Binary search to find subinterval for evaluation point
    int whichInterval = binarySearch(numberOfPoints, points, evaluationPoint);
