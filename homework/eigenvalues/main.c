...
int main(){
    ...
    for(int k=0; k<hamilSize; k++){
        int a_1 = gsl_vector_get(indexForSort,0); //We need to pull out the eigenvectors corresponding to the eigenvalues (lowest energy) So we keep track of the index vector to find right eigenvectors
        int a_2 = gsl_vector_get(indexForSort,1);
        int a_3 = gsl_vector_get(indexForSort,2);
        fprintf(psiPlot,"%g %g %g %g %g %g %g\n",(k+1)/(double) (hamilSize+1), 3.2*gsl_matrix_get(W,k,a_1), 3.2*gsl_matrix_get(W,k,a_2), 3.2*gsl_matrix_get(W,k,a_3),sin(s*M_PI*(k+1)),sin(2*s*M_PI*(k+1)),-sin(3*s*M_PI*(k+1)));
